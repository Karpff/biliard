      ...
      for(let i=0;i<balls.length;i++)
      {
        if(balls[i] != this && !balls[i].falling && !(balls[i] in this.ignoring) && !(this in balls[i].ignoring))
        {
          ...
          if(distance<ballSize*2)
          {
            ...
            this.spd.add(new THREE.Vector2(Math.cos(collisionAngle+Math.PI),Math.sin(collisionAngle+Math.PI)).multiplyScalar(directionalForce*0.3))
            balls[i].spd.add(new THREE.Vector2(Math.cos(collisionAngle),Math.sin(collisionAngle)).multiplyScalar(directionalForce*0.3));
            this.spd.clampLength(0,2);
            balls[i].spd.clampLength(0,2);
          }
